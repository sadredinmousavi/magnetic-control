...
def _scale_text(points):
    """Fit the text into an 8 cm width without distorting its proportions."""
    scaled = np.asarray(points, dtype=float).copy()
    minimum = scaled.min(axis=0)
    maximum = scaled.max(axis=0)
    center = 0.5 * (minimum + maximum)
    uniform_scale = 0.080 / (maximum[0] - minimum[0])
    return (scaled - center) * uniform_scale
...
INITIAL_ROBOT_POSITIONS = TEXT_POINTS[0] + np.column_stack((
    0.003 * np.cos(_robot_angles),
    0.003 * np.sin(_robot_angles),
))
